...
import ChannelSalesTable from '@/components/reports/channel/ChannelSalesTable';
import ChannelSalesChart from '@/components/reports/channel/ChannelSalesChart';
...
import { useSimpleChannelData } from '@/hooks/useSimpleChannelData';
...
const ChannelReport = () => {
  ...
  const { 
    ...
    error,
    isError
  } = useSimpleChannelData(fromDate, toDate);

  // Filter data based on selected channel
  const filteredData = selectedChannel 
    ? { 
        channelData: data?.channelData.filter(ch => ch.channel === selectedChannel) || [],
        months: data?.months || [] 
      }
    : data;
  ...
  useEffect(() => {
    if (isError && error) {
      ...
      console.error('Channel report error:', error);
    }
  }, [isError, error]);
      ...
      <main className="container py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold md:text-3xl">Channel Sales Report</h1>
          </div>
        </div>
        
        <Card>
          <CardHeader>
            <CardTitle>Channel Monthly Sales</CardTitle>
            <CardDescription>
              View monthly sales data for each channel
